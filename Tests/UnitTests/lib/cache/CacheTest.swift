...
final class CacheTest: XCTestCase {
    func testPutAndGetEntry() throws {
        let cache = try Cache<String>(entryEvictionTimeMillis: 1, maxEntries: 1)
    ...
    func testDropOldestEntryWhenFull() throws {
        let cache = try Cache<String>(entryEvictionTimeMillis: 1, maxEntries: 2)
    ...
    func testInvalidateOutdatedKeys() throws {
        ...
        let cache = try Cache<String>(entryEvictionTimeMillis: evictionTime, maxEntries: 4)
